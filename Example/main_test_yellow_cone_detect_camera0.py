...
if __name__ == "__main__":
    ...
    Yellow_Cone = object.Object('Yellow_Cone', 30, 30)
    logger.info(f'{Yellow_Cone.get_minsize()}')
    Cone_Detect = object.Detect(Yellow_Cone, 'Cone_Detect', 350, 5)
    Yellow_Cone.set_color(np.array([20, 100, 100]), np.array([30, 255, 255]))
    if not cap.isOpened():
        logger.info("Camera 0 Open Failed")
        exit()
    while True:
        ...
        cv2.putText(frame, "Cones Count:" + str(Cone_Detect.get_num()), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 240, 10), 5)
