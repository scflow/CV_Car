...
# Detect 锥桶
yellow_cone_detect = Detect(yellow_cone, 'yellow_cone_detect', 70, 13)
blue_cone_detect = Detect(blue_cone, 'blue_cone_detect', 70, 13)
red_cone_detect = Detect(red_cone, 'red_cone_detect', 70, 13)
...
# Detect A4纸
blue_A4_detect = Detect(blue_A4, 'blue_A4_detect', 70, 20)
red_A4_detect = Detect(red_A4, 'red_A4_detect', 70, 20)
...
# 检测函数以及实现
def ZebraCross_find(img, area_threshold=2):
    # 检测白色范围
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    _, thresh = cv2.threshold(gray, 180, 255,cv2.THRESH_BINARY)
    '''
    lower_color = np.array([0, 0, 46])  # 颜色下限
